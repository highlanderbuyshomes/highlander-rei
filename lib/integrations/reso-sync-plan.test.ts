...
import { BOOTSTRAP_STATUSES, OVERLAP_MS, resolveSyncPlan, SCOPE_VERSION } from "./reso-sync-plan";
...
describe("resolveSyncPlan", () => {
  it("bootstraps when there are no runs", () => {
    ...
    expect(p.scope).toEqual({ statuses: BOOTSTRAP_STATUSES });
  ...
  it("runs incremental after a completed run at the current scopeVersion", () => {
    const p = resolveSyncPlan({ status: "completed", rawMeta: { watermark: wm, scopeVersion: 2 } }, now);
    expect(p.kind).toBe("incremental");
    expect(p.scope).toEqual({ modifiedSince: new Date(new Date(wm).getTime() - OVERLAP_MS).toISOString() });
  ...
  it("treats completed_with_errors at the current scopeVersion as incremental", () => {
    const p = resolveSyncPlan({ status: "completed_with_errors", rawMeta: { watermark: wm, scopeVersion: 2 } }, now);
    expect(p.kind).toBe("incremental");
  });
  ...
  it("resumes a partial run at the current scopeVersion with a resumeUrl", () => {
    ...
    const p = resolveSyncPlan({ status: "partial", rawMeta: { watermark: wm, resumeUrl: "https://x/next", scope, scopeVersion: 2 } }, now);
    expect(p.kind).toBe("resume-partial");
    expect(p.scope).toEqual(scope);
    expect(p.control.resumeUrl).toBe("https://x/next");
    expect(p.control.meta).toEqual({ watermark: wm, scope, scopeVersion: 2 });
  });

  it("bootstraps a partial run missing its resumeUrl", () => {
    expect(resolveSyncPlan({ status: "partial", rawMeta: { watermark: wm, scope: {}, scopeVersion: 2 } }, now).kind).toBe("bootstrap");
  });
});
