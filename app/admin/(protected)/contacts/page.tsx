...
function TypeBadge({ type }: { type: string | null }) {
  if (!type) return <span style={{ fontSize: "11px", color: "#64748b" }}>—</span>;
  const cfg = TYPE_CONFIG[type] ?? { label: type, bg: "#f0efeb", color: "#475569", border: "#c7d0d8" };
  return (
...
export default async function ContactsPage({
  ...
  return (
    <div className="admin-workspace-shell" style={{ display: "flex", minHeight: "calc(100vh - 56px)" }}>
      ...
      {/* ── Sidebar ── */}
      <aside className="admin-workspace-sidebar" style={{ width: "236px", background: "#ffffff", borderRight: "1px solid #e1e7ec", display: "flex", flexDirection: "column", flexShrink: 0, paddingBottom: "24px" }}>

        <div className="admin-workspace-sidebar-section" style={{ padding: "16px 14px 12px" }}>
          <Link href="/admin/contacts/new" style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "8px", background: "#12161c", color: "#ffffff", padding: "11px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: 600, textDecoration: "none", letterSpacing: "0.3px" }}>
        ...
        <div className="admin-workspace-sidebar-section" style={{ padding: "8px 10px 0" }}>
          <div className="admin-workspace-sidebar-label" style={{ fontSize: "9px", fontWeight: 700, color: "#64748b", letterSpacing: "1.8px", textTransform: "uppercase", padding: "0 8px", marginBottom: "4px" }}>CONTACTS</div>
          {FILTERS.map(({ key, label, count, icon }) => {
            const active = filter === key;
            return (
              <Link key={key} href={filterHref(key)} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", borderRadius: "6px", marginBottom: "1px", background: active ? "#f2f5f8" : "transparent", textDecoration: "none", borderLeft: active ? "2px solid #12161c" : "2px solid transparent", color: active ? "#12161c" : "#64748b" }}>
        ...
        <div className="admin-workspace-sidebar-divider" style={{ margin: "20px 14px 0", borderTop: "1px solid #e1e7ec" }} />
        <div className="admin-workspace-sidebar-section" style={{ padding: "12px 10px 0" }}>
          <Link href="/admin/agreements" style={{ display: "flex", alignItems: "center", gap: "10px", padding: "8px 10px", borderRadius: "6px", textDecoration: "none", color: "#64748b" }}>
      ...
      {/* ── Main ── */}
      <main className="admin-workspace-main" style={{ flex: 1, background: "#eef2f6", display: "flex", flexDirection: "column" }}>

        <div className="admin-workspace-header" style={{ padding: "28px 32px 0", borderBottom: "1px solid #e1e7ec", background: "#ffffff" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "20px" }}>
            ...
            <span style={{ fontSize: "12px", color: "#64748b" }}>{contacts.length} contact{contacts.length !== 1 ? "s" : ""}</span>
          </div>
          ...
          <form method="GET" action="/admin/contacts" style={{ display: "flex", gap: "8px", paddingBottom: "20px" }}>
            ...
            <div style={{ display: "flex", flex: 1, alignItems: "center", background: "#eef2f6", border: "1px solid #c7d0d8", borderRadius: "8px", padding: "0 14px", gap: "8px" }}>
              ...
              <input name="q" defaultValue={q ?? ""} placeholder="Search name, email, company…" style={{ flex: 1, border: "none", background: "transparent", fontSize: "13px", color: "#12161c", outline: "none", padding: "10px 0", fontFamily: "inherit" }} />
            </div>
            <button type="submit" style={{ padding: "0 20px", background: "#12161c", color: "#ffffff", border: "none", borderRadius: "8px", fontSize: "12.5px", fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>Search</button>
            {hasSearch && (
              <Link href="/admin/contacts?type=all" style={{ padding: "0 14px", background: "#ffffff", color: "#64748b", border: "1px solid #c7d0d8", borderRadius: "8px", fontSize: "12.5px", display: "flex", alignItems: "center", textDecoration: "none" }}>✕ Clear</Link>
            )}
          </form>
        </div>
          ...
          {contacts.length === 0 ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "80px 40px", textAlign: "center" }}>
              <svg width="64" height="64" viewBox="0 0 64 64" fill="none" style={{ marginBottom: "20px", opacity: 0.3 }}>
                <circle cx="32" cy="24" r="12" stroke="#12161c" strokeWidth="2"/>
                <path d="M8 56c0-13.255 10.745-24 24-24s24 10.745 24 24" stroke="#12161c" strokeWidth="2" strokeLinecap="round"/>
              </svg>
              <div style={{ fontFamily: "var(--font-display), serif", fontSize: "20px", color: "#12161c", letterSpacing: "2px", marginBottom: "8px" }}>NO CONTACTS FOUND</div>
              ...
              <Link href="/admin/contacts/new" style={{ display: "inline-flex", alignItems: "center", gap: "8px", background: "#12161c", color: "#ffffff", padding: "11px 24px", borderRadius: "8px", fontSize: "13px", fontWeight: 600, textDecoration: "none" }}>
                <svg width="11" height="11" viewBox="0 0 11 11" fill="none"><path d="M5.5 1v9M1 5.5h9" stroke="currentColor" strokeWidth="2" strokeLinecap="round"/></svg>
                Add Contact
              </Link>
            </div>
          ) : (
            <div className="admin-workspace-table" style={{ background: "#ffffff", border: "1px solid #e1e7ec", borderRadius: "12px", overflow: "hidden" }}>
              <div className="admin-workspace-table-row" style={{ display: "grid", gridTemplateColumns: "1.4fr 1.4fr 1.2fr 1fr 80px 60px 120px", padding: "10px 20px", background: "#eef2f6", borderBottom: "1px solid #e1e7ec" }}>
              ...
              {contacts.map((c, i) => {
                ...
                return (
                  <div key={c.id} className="admin-workspace-table-row" style={{ display: "grid", gridTemplateColumns: "1.4fr 1.4fr 1.2fr 1fr 80px 60px 120px", padding: "14px 20px", borderBottom: i < contacts.length - 1 ? "1px solid #f0efeb" : "none", alignItems: "center", background: "#ffffff" }}>
                    <div>
                      <div style={{ fontSize: "13px", color: "#12161c", fontWeight: 500 }}>{displayName}</div>
                    ...
                    <div style={{ fontSize: "12px", color: "#475569" }}>{c.phone ?? "—"}</div>
                    <div style={{ fontSize: "12px", color: "#475569" }}>{c.company ?? "—"}</div>
                    <div><TypeBadge type={c.contactType ?? null} /></div>
                    <div style={{ fontSize: "12px", color: "#64748b", textAlign: "center" }}>{c._count.signers}</div>
                    <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end" }}>
                      <Link href={`/admin/contacts/new?edit=${c.id}`} style={{ fontSize: "12px", color: "#12161c", textDecoration: "none", padding: "5px 12px", border: "1px solid #c7d0d8", borderRadius: "6px", background: "#ffffff" }}>
                        Edit
                      </Link>
